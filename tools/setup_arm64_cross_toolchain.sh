#!/usr/bin/env bash
# tools/setup_arm64_cross_toolchain.sh — one-time bootstrap for cross-compiling KStars
# to aarch64 on a faster x86_64 machine, then finishing the build locally via distcc.
#
# This is a SEPARATE, one-time (or rare) setup step from tools/distcc_build.py, which
...
# a prebuilt one matched to what's actually running on the Pi).
#
...
#
# ── Requirements ─────────────────────────────────────────────────────────────────────
#   - Local machine: aarch64 (the Pi/CM4 itself), passwordless sudo, `distcc` installed
#     (installed automatically below via /etc/stellarmate/pacman-build.conf if missing).
...
#
# ── Usage ────────────────────────────────────────────────────────────────────────────
#   tools/setup_arm64_cross_toolchain.sh --host workshop
#   tools/setup_arm64_cross_toolchain.sh --host workshop --force   # redo even if present
#
...
#
#        mkdir -p ~/Projects/build/kstars && cd ~/Projects/build/kstars
#        cmake -G Ninja \
#          -DCMAKE_INSTALL_PREFIX=/usr -DCMAKE_BUILD_TYPE=Debug -DBUILD_TESTING=Off \
#          -DBUILD_WITH_QT6=ON -DCCACHE_SUPPORT=ON \
...
#      squeezed by an oversized -j during that step.)
#
#   2. Every build after that:
#
#        tools/distcc_build.py --host workshop --build-dir ~/Projects/build/kstars \
#          --slots 24 --jobs 6
#
...
#      -j jobs total, and several build steps are inherently local-only regardless of
#      distcc (Qt's moc/uic/rcc codegen, the final archive+link). Sizing -j from the
#      remote's core count oversubscribes the local machine badly during those phases
#      (verified: -j24 on a 4-core Pi during the moc phase spiked load to ~10 with the
#      remote sitting idle, before any real compiling had even started). Once real
#      compiles begin, each local ninja job is mostly waiting on the network, not burning
#      a full local core, so this is a safe, conservative number, not an efficiency
#      compromise -- distcc's remote-side capacity is unaffected by it.
#
#   3. To install what you built: sudo cmake --install ~/Projects/build/kstars
#
