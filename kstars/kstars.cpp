...
KStars::KStars(bool doSplash, bool clockrun, const QString &startdate, const bool liveStacker)
    : KXmlGuiWindow(), StartClockRunning(clockrun), StartDateString(startdate)
{
    ...
    QString title = liveStacker ? i18nc("@title:window", "KStars Live Stacker") : i18nc("@title:window", "KStars");
    ...
    QString welcomeText = (liveStacker) ? "Welcome to KStars Live Stacker" : "Welcome to KStars";
    qCInfo(KSTARS) << welcomeText << KSTARS_VERSION << KSTARS_BUILD_RELEASE;
...
KStars *KStars::createInstance(bool doSplash, bool clockrun, const QString &startdate, const bool liveStacker)
{
    delete pinstance;
    // pinstance is set directly in constructor.
    new KStars(doSplash, clockrun, startdate, liveStacker);
    Q_ASSERT(pinstance && "pinstance must be non NULL");
    return pinstance;
}

bool KStars::launchLiveStackerStandalone()
{
    // Setup basic KAboutData
    ...
    // Uncomment to provide a pause in process startup to allow attaching a debugger
    //QMessageBox::information(nullptr, "Attach Debugger", "Live Stacker mode");

    // Create main KStars window (does not show it in liveStacker mode)
    KStars *mainWindow = KStars::createInstance(false, false, "", true);
    if (!mainWindow)
        return false;

    // Show the main window (important for event loop and button response) but then hide it as we don't need it
    mainWindow->show();
    mainWindow->hide();
    ...
    Q_ASSERT(data);
    QObject::connect(data, &KStarsData::progressText, data, &KStarsData::slotConsoleMessage);
    data->colorScheme()->loadFromConfig();
    data->clock()->setUTC(KStarsDateTime::currentDateTimeUtc());

    // Set up appearance consistent with KStars
    if (!QIcon::hasThemeIcon(QLatin1String("kstars_flag")))
        KSTheme::Manager::instance()->setIconTheme(KSTheme::Manager::BREEZE_DARK_THEME);
    else
        KSTheme::Manager::instance()->setIconTheme(KSTheme::Manager::BREEZE_THEME);

    // Launch FITSViewer in live stacking mode
    auto viewer = new FITSViewer(mainWindow, FITSViewer::Mode::LiveStacking);
    viewer->show();
    return true;
}
...
const QSharedPointer<FITSViewer> &KStars::createFITSViewer(const bool forceCreate)
{
    ...
    {
        QSharedPointer<FITSViewer> newFITSViewer(new FITSViewer(Options::independentWindowFITS() ? nullptr :
                KStars::Instance(), FITSViewer::Mode::Full), &QObject::deleteLater);
