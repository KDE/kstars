...
void SchedulerSafetyMonitor::initStandaloneSafetyMonitor(const QString &connectionString)
{
    ...
    emit newLog(i18n("Initializing standalone safety monitor monitoring for %1@%2:%3",
                     m_SafetyMonitorDeviceName, m_SafetyMonitorHostname, m_SafetyMonitorPort));
    ...
    connect(m_SafetyMonitorClientManager, &ClientManager::started, this, &SchedulerSafetyMonitor::handleSafetyMonitorClientStarted,
            Qt::UniqueConnection);
    connect(m_SafetyMonitorClientManager, &ClientManager::failed, this, &SchedulerSafetyMonitor::handleSafetyMonitorClientFailed,
            Qt::UniqueConnection);
    connect(m_SafetyMonitorClientManager, &ClientManager::terminated, this, &SchedulerSafetyMonitor::handleSafetyMonitorClientTerminated,
            Qt::UniqueConnection);
...
void SchedulerSafetyMonitor::parseConnectionString(const QString &connectionString)
{
    ...
    if (parts.size() != 2)
    {
        emit newLog(i18n("Warning: Invalid safety monitor connection string format. Expected 'host:port'"));
...
void SchedulerSafetyMonitor::connectToStandaloneSafetyMonitor()
{
    ...
    emit newLog(i18n("Connecting to standalone safety monitor at %1:%2...", m_SafetyMonitorHostname, m_SafetyMonitorPort));
...
void SchedulerSafetyMonitor::handleNewGenericDevice(const QSharedPointer<ISD::GenericDevice> &device)
{
    ...
    if (device->getDeviceName() == m_SafetyMonitorDeviceName)
    {
        ...
        if (m_StandaloneSafetyMonitorDevice)
        {
            ...
            connect(m_StandaloneSafetyMonitorDevice, &ISD::GenericDevice::propertyUpdated, this, &SchedulerSafetyMonitor::processSafetyProperty, Qt::UniqueConnection);

            m_StandaloneSafetyMonitorConnected = true;
            m_SafetyMonitorConnectionRetries = 0;

            // Check initial property value
            auto safetyProp = m_StandaloneSafetyMonitorDevice->getProperty("SAFETY_MONITOR");
