...
class OpticalTrainManager : public QDialog, public Ui::OpticalTrain
{
    ...
    public:
        ...
        const QVariantMap getOpticalTrain(uint32_t id) const;
        ...
        /**
         * @brief A stored train device reference that is not currently connected. May be a
         * device that is only temporarily unavailable, or one that is gone for good - this
         * cannot be determined from stored data alone, so it is left to the user to decide.
         */
        struct UnusedDevice
        {
            QString displayName;
        ...
        /**
         * @brief getUnusedDevices Collect stored device references in the current profile's
         * optical trains that match no currently-connected device.
         */
        QList<UnusedDevice> getUnusedDevices() const;

        /**
         * @brief removeUnusedDevice Clear a stale train field back to "--".
