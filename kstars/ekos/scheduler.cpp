...
{
    QUrl fileURL = QFileDialog::getOpenFileUrl(this, xi18n("Open FITS Image"), QString(), "FITS (*.fits *.fit)");
    if (fileURL.isEmpty())
        return;

    fitsEdit->setText(fileURL.path());
...
{
    QUrl fileURL = QFileDialog::getOpenFileUrl(this, xi18n("Open Sequence Queue"), QString(), xi18n("Ekos Scheduler Queue (*.esq)"));
    if (fileURL.isEmpty())
        return;

    sequenceFile->setText(fileURL.path());
}
