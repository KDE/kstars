...
void KStarsPlotWidget::drawBox( QPainter *p ) {
	...
	if ( axis(LeftAxis)->isVisible() || axis(BottomAxis)->isVisible() ) p->drawRect( PixRect ); //box outline
	...
	//Draw X Axis Label(s)
	if ( ! axis(BottomAxis)->label().isEmpty() ) {
		QRect r( 0, PixRect.height() + 2*YPADDING, PixRect.width(), YPADDING );
		p->drawText( r, Qt::AlignCenter | Qt::TextDontClip, axis(BottomAxis)->label() );
	}
	...
	if ( ! axis(LeftAxis)->label().isEmpty() ) {
		...
		p->drawText( r, Qt::AlignCenter | Qt::TextDontClip, axis(LeftAxis)->label() ); //draw the label, now that we are sideways
