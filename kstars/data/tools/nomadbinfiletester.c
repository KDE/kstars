...
/***************************************************************************
     nomadbinfiletester.c - Check star catalog (binary) files for errors
                             -------------------
...
int verifyStarData( FILE *f ) {
    ...
    for( trixel = 0; trixel < ntrixels; ++trixel ) {
        ...
        for( i = 0; i < nstars; ++i ) {
            ...
            if( mag != -5000 && ( ( new_mag - mag ) > 20 && mag < 1250 ) || new_mag < mag ) { // TODO: Make sensible magnitude limit (1250) user specifiable
                // TODO: Enable byteswapping
                fprintf( stderr, "\n\tEncountered jump of %f at star #%d in trixel %d from %f to %f.", ( new_mag - mag ) / 1000.0, i, trixel, mag / 1000.0, new_mag / 1000.0 );
                ++nerr_trixel;
            }
            mag = new_mag;
            if( mag > realFaintMag ) {
