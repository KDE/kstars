...
{
    m_Values << 0.01 << 0.02 << 0.05 << 0.1 << 0.2 << 0.25 << 0.5 << 1 << 1.5 << 2 << 2.5 << 3 << 5 << 6 << 7 << 8 << 9 << 10 <<
...
void NonLinearDoubleSpinBox::updateRecommendedValues()
{
    ...
    m_idx = m_Values.indexOf(
                value());  //This will update the _idx variable to the index of the new value.  It will search for current value in the new list or set it to negative 1 if it isn't in the list.
    setRange(m_Values.first(), m_Values.last());
