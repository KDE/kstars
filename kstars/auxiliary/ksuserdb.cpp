...
// Add sample profile
            ...
            if (currentDBVersion < "2.8.1")
            {
                ...
                QSqlQuery query(userdb_);
                if (!query.exec("DROP TABLE hips"))
                    qCWarning(KSTARS) << query.lastError();
                if (!query.exec("CREATE TABLE IF NOT EXISTS hips (ID TEXT DEFAULT NOT NULL PRIMARY KEY "
                ...
                {
                    if (!query.exec("INSERT INTO hips (ID, obs_title, obs_description, hips_frame, hips_tile_width, hips_tile_format, hips_service_url, moc_sky_fraction )"
