...
// Add sample profile
            ...
            // If prior to 2.8.1 upgrade database to add HIPS sources
            if (currentDBVersion < "2.8.1")
            {
                // To get all lists
                // http://alasky.unistra.fr/MocServer/query?hips_url=*&dataproduct_type=!catalog&dataproduct_type=!cube&moc_sky_fraction=1&get=record
                QSqlQuery query(userdb_);
                if (!query.exec("CREATE TABLE IF NOT EXISTS hips (id INTEGER DEFAULT NULL PRIMARY KEY "
                                "AUTOINCREMENT, title TEXT NOT NULL, description TEXT NOT NULL, frame TEXT NOT NULL,"
                                "format TEXT NOT NULL, url TEXT NOT NULL, hipsorder INTEGER, size INTEGER,"
                                "skyfraction REAL, timestamp INTEGER)"))
                    qCWarning(KSTARS) << query.lastError();
                // Add default data
                else
                {
                    if (!query.exec("INSERT INTO hips (title, description, frame, format, url, hipsorder, size, skyfraction, timestamp )"
                                    "VALUES ('DSS colored', 'Color composition generated by CDS. This HiPS survey is based on 2 others HiPS surveys,"
...
//tables.append("CREATE TABLE custom_driver (id INTEGER DEFAULT NULL PRIMARY KEY AUTOINCREMENT, drivers TEXT NOT NULL, profile INTEGER NOT NULL, FOREIGN KEY(profile) REFERENCES profile(id))");
    ...
    tables.append("INSERT INTO hips (title, description, frame, format, url, hipsorder, size, skyfraction, timestamp )"
                  "VALUES ('DSS colored', 'Color composition generated by CDS. This HiPS survey is based on 2 others HiPS surveys,"
                  " respectively DSS2-red and DSS2-blue HiPS, both of them directly generated from original scanned plates downloaded"
                  " from STScI site. The red component has been built from POSS-II F, AAO-SES,SR and SERC-ER plates. The blue component"
                  " has been build from POSS-II J and SERC-J,EJ. The green component is based on the mean of other components. Three"
...
void KSUserDB::AddHIPSSource(const QVariantMap &oneSource)
{
    userdb_.open();
    QSqlTableModel HIPSSource(0, userdb_);
    HIPSSource.setTable("HIPSSource");
    HIPSSource.select();
    ...
    for (QVariantMap::const_iterator iter = oneSource.begin(); iter != oneSource.end(); ++iter)
        record.setValue(iter.key(), iter.value());
...
bool KSUserDB::DeleteHIPSSource(const QString &title)
{
    userdb_.open();
    QSqlTableModel HIPSSource(0, userdb_);
    HIPSSource.setTable("hips");
    HIPSSource.setFilter("title = \'" + title + "\'");
...
void KSUserDB::GetAllHIPSSources(QList<QVariantMap> &HIPSSources)
{
    HIPSSources.clear();
