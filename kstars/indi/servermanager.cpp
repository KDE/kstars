...
bool ServerManager::start()
{
    ...
    {
        qCCritical(KSTARS_INDI) << "Unable to create INDI FIFO file: " << fifoFile << Qt::endl;
        return false;
    }
...
void ServerManager::stopDriver(DriverInfo *dv)
{
    ...
    if (dv->getUniqueLabel().isEmpty() == false)
        out << "stop " << dv->getExecutable() << " -n \"" << dv->getUniqueLabel() << "\"" << Qt::endl;
    else
        out << "stop " << dv->getExecutable() << Qt::endl;
