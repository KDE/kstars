...
void TestPolarAlign::testRunPAA()
{
    ...
    runPAA(
    {
        ...
        //1880, 1565, 1853, 1541 // mount axis x,y  pole center in x,y
        1880, 1565, 1853, 1553
    });
    ...
    runPAA(
    {
        ...
        // Here are below the various alternatives are different points
        // selected in the UI when polar aligning.
        // 1813, 1747, 1800, 1753
        2328, 1760, 2315, 1765,
    });
...
void TestPolarAlign::testRotate_data()
{
    ...
    QTest::newRow("") <<    1.0 <<   1.0  << 0.3  << 0.4 <<     1.3002   <<  1.3999;
    ...
    QTest::newRow("") <<  -30.0 <<  60.0  << 0.0  << 3.0 <<   -32.855841 << 62.5608440;
    ...
    QTest::newRow("") <<  -30.0 <<  60.0  << 1.0  << -1.0 <<  -28.185523 << 59.121929;
    QTest::newRow("") <<   30.0 <<  60.0  << 1.0  << -1.0 <<   30.134229 << 59.138936;
    QTest::newRow("") << -110.0 <<  60.0  << 1.0  << -1.0 << -107.343842 << 60.311909;
    QTest::newRow("") <<  110.0 <<  60.0  << 1.0  << -1.0 <<  109.362827 << 60.345037;
