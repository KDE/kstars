...
void TestEkosMeridianFlipState::testMeridianFlip_data()
{
    ...
    for (auto loc: {"Cape Town", "Greenwich"})
    {
        ...
        QTest::newRow(QString("loc=%1 enabled=yes, capture=yes, upper=yes").arg(loc).toLatin1()) << loc << true << true << true << 0.0;
        QTest::newRow(QString("loc=%1 enabled=yes, capture=yes, upper=no").arg(loc).toLatin1()) << loc << true << true << false << 0.0;
        QTest::newRow(QString("loc=%1 enabled=yes, capture=yes, upper=yes offset=15'").arg(loc).toLatin1()) << loc << true << true << true << 0.25;
        QTest::newRow(QString("loc=%1 enabled=yes, capture=yes, upper=no offset=15'").arg(loc).toLatin1()) << loc << true << true << false << 0.25;
    }
...
void TestEkosMeridianFlipState::connectAdaptor()
{
    ...
    connect(m_Mount, &MountSimulator::newStatus, m_stateMachine, &Ekos::MeridianFlipState::setMountStatus, Qt::UniqueConnection);
    connect(m_Mount, &MountSimulator::newCoords, m_stateMachine, &Ekos::MeridianFlipState::updateTelescopeCoord, Qt::UniqueConnection);
...
void TestEkosMeridianFlipState::disconnectAdaptor()
{
    ...
    disconnect(m_stateMachine, &Ekos::MeridianFlipState::newMountMFStatus, this, &TestEkosMeridianFlipState::meridianFlipMountStatusChanged);
